...
class LeastSquaresTopNSketch(TopNCountMinSketch):
    ...
    def most_common(self, k=None):
        r"""Generate a better estimate for the top k most-common entries using the least squares estimator \
        ...
        Formally, we construct :math:`\vec{b}` as a concatenation of the tables rows, implemented by \
        table.to_vector(). The size of :math:`\vec{b}` is thus (table.depth * table.width) x 1.

        We construct :math:`A` as having the same number or rows as :math:`\vec{b}`, and having \
        :math:`k + 1` columns, using the following values:

        for :math:`i \in \{0, 1, ..., \text{table.width} - 1\}` \
        and :math:`j \in \{0, 1, ..., \text{table.depth} - 1 \}`,

        we set \ :math:`A_{\text{table.depth} * i + j + 1, l} = 1` if either the item :math:`x_l` is hashed into \
        :math:`T[i][j]`, or if :math:`l = \text{table.width} + 1`, and otherwise we leave it as 0.

        We then solve using least-squares estimation (using the pseudoinverse) :math:`A\vec{x}=\vec{b}`, \
        sort the results in a descending fashion (discarding the noise), and report the top k entries.
