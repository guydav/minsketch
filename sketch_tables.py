# -*- coding: utf-8 -*-
"""Implementation of the different table styles. Four are currently implemented - backing by a simple python list,
backing by python arrays (which are more efficient, but provide a very similar interface), backing by numpy matrices
(which turned out to be rather inefficient), and backing by bitarrays (which saves a lot of memory, but incurs a
significant performance penalty.

In the future I should implement a table in C, bridge it to Python, and call it a day.
"""
import abc
import numpy
import bitarray
...
class SketchTable(object):
    ...
    """An interface representing what a table for a sketch should be able to do. It actually also handles the
    entire implementation for things that conform to the python list interface.
    """
    @abc.abstractmethod
    ...
    def increment(self, depth, index, value=1):
        ...
        Also tracks how many total values have been inserted.
        :param depth: The depth (which array/table) to return from
        :param index: The index within this current array
    ...
    def row_iter(self):
        ...
        count-mean-min with conservative update (CMM-CU)'s summation over each row
    ...
    def decrement_all(self, upper_bound=None, lower_bound=0):
        """A decrement-all operation designed to support lossy counting - see Goyal and Daumé (2011):
        http://www.umiacs.umd.edu/~amit/Papers/goyalLCUSketchAAAI11.pdf

        When this function is called, it iterates through all items, and decrements whichever match the bounds.
        The un-intuitive ordering of arguments stems from the fact that in many cases it makes sense to change the
        upper bound (LCU-1, LCU-WS, LCU-SWS), while none I've seen so far attempt to change the lower bound.
...
class BitarrayBackedSketchTable(SketchTable):
    ...
    def decrement_all(self, upper_bound=POSITIVE_INFINITY, lower_bound=0):
        ...
        :return: None; all relevant items decremented
        """
        # TODO: If feeling adventurous, benchmark against unpacking all values, and then decrementing the necessary ones
