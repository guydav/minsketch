...
class NaiveUpdateStrategy(object):
    """ This could clearly be a function, rather than a class. Why make it a class, then? Two reasons:
    ...
    3) I already ended up using its class-ness, to couple error estimation strategies here.
    """
    ...
    def count_mean_error_estimates(self, table, values):
        """Provide an error estimate, using the total insertions into the table as a baseline
        :param table: The table being updated
    ...
    def _error_estimates(self, table, values, baselines):
        return [(baseline - value) / (table.width - 1) for value, baseline in izip(values, baselines)]
...
class ConservativeUpdateStrategy(NaiveUpdateStrategy):
    ...
    def __call__(self, table, hashes, count):
        ...
        insensible, and might lead to unexpected results.
        :param table: The table being updated
        :param hashes: The hashes of the current item, each per row in the table
        :param count: The count to increment by
        :return: The new minimal count for this item being updated
    ...
    def count_mean_error_estimates(self, table, values):
        """Error estimates for the count-min-mean, but in this case, we used the row-wise sum
        as an estimator for the entry in that row, rather than the entire total.
        :param table: The table being updated
        :param values: Values of the item we return error estimates for
        :return: Error estimates for each of these values
        """
